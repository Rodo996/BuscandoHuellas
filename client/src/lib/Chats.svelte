<script>
    ...
    const COLORES = [
        ...
        "#ED1A1A", "#1ABFD2", "#24FB32", "rgba(13,59,102,0.6)"
    ];
    ...
    onMount(async () => {
        if (!user_id) { cargando = false; return; }
        try {
            const res = await fetch(`${API}/chats/${user_id}`);
            if (!res.ok) throw new Error('Error al cargar chats');
            const data = await res.json();

            // Mapear la respuesta al formato que espera el template y Chat.svelte
            listaContactos = data.map((chat, i) => ({
...
<div class="app-container">
    ...
    {:else if error}
        <p style="text-align:center; padding:24px; font-family:Poppins; color:#C2410C;">
            {error}
        </p>
    {:else if listaContactos.length === 0}
        <p style="text-align:center; padding:24px; font-family:Poppins; color:#6B7280;">
            No tienes chats aún
        </p>
    {:else}
        {#each listaContactos as contacto}
            <div class="contact-item" on:click={() => abrirChat(contacto)}>
                ...
                <div style="flex:1; margin-left:16px; display:flex; flex-direction:column;">
                    ...
                    {#if contacto.ultimoMensaje}
                        <span style="font-size:12px; color:#6B7280; font-family:Inter;">
