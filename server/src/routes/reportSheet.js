...
router.get("/:id", async (req, res) => {
    try {
        ...
        // Datos base traidos por la view
        const [[row]] = await pool.query(
            `SELECT * FROM vw_posts_completos WHERE post_id = ?`, [id]
        );
        if (!row) return res.status(404).json({ error: "No encontrada" });

        // Colores van en una query separada por la relación M:N
        const [colores] = await pool.query(
            `SELECT c.color_id AS id, c.color_name AS color_name, c.hex_code AS hex
       FROM Pet_Colors pc
       JOIN Colors c ON pc.color_id = c.color_id
       WHERE pc.pet_id = (SELECT pet_id FROM Posts WHERE post_id = ?)`, [id]
        );

        // Discapacidades prosigue igual que con Colores
        const [discapacidades] = await pool.query(
            `SELECT d.disability_id AS id, d.disability_name AS nombre
       FROM Pet_Disabilities pd
       JOIN Disabilities d ON pd.disability_id = d.disability_id
       WHERE pd.pet_id = (SELECT pet_id FROM Posts WHERE post_id = ?)`, [id]
        );
        ...
        res.json({
            ...
            colores,
            discapacidades,
            esCruza: row.is_mix === 1,
            tieneCola: row.has_tail === 1,
        });
    } catch (err) {
        res.status(500).json({ error: "Error de servidor", details: err.message });
    }
