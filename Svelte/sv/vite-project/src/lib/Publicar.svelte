<script>
  ...
  const opcionesColor = [
    ...
    { nombre: "Café", hex: "#795548" },
    { nombre: "Café claro", hex: "#d4a96a" },
    { nombre: "Beige/Crema", hex: "#f5f0dc" },
    { nombre: "Naranja", hex: "#ff7043" },
    { nombre: "Amarillo", hex: "#fdd835" },
    { nombre: "Rojo", hex: "#e53935" },
    { nombre: "Azul", hex: "#1e88e5" },
    { nombre: "Verde", hex: "#43a047" },
    { nombre: "Tricolor", hex: "#000000" }, // especial
  ];
  ...
  $: razasDisponibles = especieSeleccionada
    ? (razasPorEspecie[especieSeleccionada] ?? [])
    : [];
  $: if (especieSeleccionada) razaSeleccionada = "";
  ...
  const opcionesDiscapacidad = [
    ...
    "Ciego de ambos ojos",
    "Sordo",
    "Cojera / pata lastimada",
    "Amputación",
    "Ninguna",
  ];

  function toggleDiscapacidad(opcion) {
    if (discapacidadesSeleccionadas.includes(opcion)) {
      ...
      <div class="field-group">
        ...
        >
          <select class="select-native" bind:value={especieSeleccionada}>
            ...
            {#each Object.keys(razasPorEspecie) as esp}
          ...
          <div class="color-dropdown">
            {#each opcionesColor as opcion}
              <button
                ...
                class:selected={colorSeleccionado?.nombre === opcion.nombre}
                on:click={() => seleccionarColor(opcion)}
              >
                {#if opcion.nombre === "Tricolor"}
                  <span
                    class="color-dot"
                    style="background: linear-gradient(135deg, #1a1a1a 33%, #d4a96a 33% 66%, #f5f5f5 66%);"
                  ></span>
                {:else}
                  <span class="color-dot" style="background: {opcion.hex};"
                  ></span>
                {/if}
                <span class="color-dropdown-label">{opcion.nombre}</span>
                {#if colorSeleccionado?.nombre === opcion.nombre}
                  <svg
                    width="12"
                    height="10"
                    viewBox="0 0 12 10"
    ...
    <div class="field-group">
      ...
      >
        {#each opcionesDiscapacidad as opcion}
          {@const checked = discapacidadesSeleccionadas.includes(opcion)}
          <button
            type="button"
            class="checkbox-item"
            class:checked
    ...
    <div class="two-col">
      <div class="field-group">
        ...
        <input
          ...
          bind:value={fecha}
        />
        {#if intentoEnvio && errores.fecha}<span class="error-msg"
            >Obligatorio</span
          >{/if}
      </div>
      <div class="field-group">
